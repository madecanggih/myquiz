...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($question_err) && empty($option1_err) && empty($option2_err) && empty($option3_err) && empty($option4_err) && empty($option5_err) && empty($answer_err) && empty($image_err) ){
        ...
        $sql = "INSERT INTO tb_question (question_name, question_image, option1, option2, option3, option4, option5, answer) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssi", $param_question, $param_image, $param_option1, $param_option2, $param_option3, $param_option4, $param_option5, $param_answer);
...
@media (max-width: 992px) {
  ...
  <body style="font-family: 'Roboto', sans-serif">
    ...
    <div class="wrapping">
      ...
      <div class="container">
          <center><h2>Tambah Soal</h2></center>
          <br>
          <?php if(isset($_GET['msg'])){
            if ($_GET['msg'] === "success"){
              echo "<div class='alert alert-success' role='alert'>Berhasil menambahkan soal!</div>";
            }
            else{
      ...
      <div class="form-group">
        ...
        <textarea class="form-control <?php echo (!empty($question_err)) ? 'is-invalid' : ''; ?>" name="question_name" rows="5"><?php echo $question_name; ?></textarea>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control <?php echo (!empty($option1_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $option1; ?>" name = "option1"/>
      <span class="help-block" style="color: red;"><?php echo $option1_err; ?></span>
    </div>

    <div class="form-group">
      <label for="option2">Opsi 2</label>
      <input type="text" class="form-control <?php echo (!empty($option2_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $option2; ?>" name = "option2"/>
      <span class="help-block" style="color: red;"><?php echo $option2_err; ?></span>
    </div>

    <div class="form-group">
      <label for="option3">Opsi 3</label>
      <input type="text" class="form-control <?php echo (!empty($option3_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $option3; ?>" name = "option3"/>
      <span class="help-block" style="color: red;"><?php echo $option3_err; ?></span>
    </div>

    <div class="form-group">
      <label for="option4">Opsi 4</label>
      <input type="text" class="form-control <?php echo (!empty($option4_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $option4; ?>" name = "option4"/>
      <span class="help-block" style="color: red;"><?php echo $option4_err; ?></span>
    </div>

    <div class="form-group">
      <label for="option5">Opsi 5</label>
      <input type="text" class="form-control <?php echo (!empty($option5_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $option5; ?>" name = "option5"/>
    ...
    <div class="form-group">
                ...
                <select class="custom-select <?php echo (!empty($answer_err)) ? 'is-invalid' : ''; ?>" name="answer">
                  ...
                  <option value="1" <?php if(!empty($answer) && $answer == 1){ echo "selected";} ?>>Opsi 1</option>
                  <option value="2" <?php if(!empty($answer) && $answer == 2){ echo "selected";} ?>>Opsi 2</option>
                  <option value="3" <?php if(!empty($answer) && $answer == 3){ echo "selected";} ?>>Opsi 3</option>
                  <option value="4" <?php if(!empty($answer) && $answer == 4){ echo "selected";} ?>>Opsi 4</option>
                  <option value="5" <?php if(!empty($answer) && $answer == 5){ echo "selected";} ?>>Opsi 5</option>
                </select>
